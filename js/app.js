...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // New features (carousel, shorts, profile)
...
async function loadChannels(silent = false) {
  ...
  try {
    ...
    // Refresh carousel with loaded channels
    if (typeof onChannelsLoaded === 'function') onChannelsLoaded();
...
async function loadIptvChannels(silent = false) {
  ...
  // Render shorts feed after IPTV loaded
  if (typeof onIptvLoaded === 'function') onIptvLoaded();
}
...
function openChannel(id, title, desc, type, owner) {
  ...
  const _siteName = window.SITE_NAME || window.location.hostname;
  history.replaceState({ channelId: id, title }, title, `/watch/${slug}`);
  document.title = `${title} — смотреть онлайн прямой эфир | ${_siteName}`;

  // Update meta for SEO
  updateMeta(`${title} — смотреть онлайн | ${_siteName}`,
...
function openIptvChannel(name, url, logo) {
  ...
  document.title = `${name} — IPTV онлайн | ${_siteName}`;
  updateMeta(`${name} — смотреть IPTV онлайн | ${_siteName}`,
...
function closePlayer() {
  ...
  history.replaceState({}, window.SITE_NAME || window.location.hostname, getFilterPath());
  const _sn = window.SITE_NAME || window.location.hostname;
  document.title = `${_sn} — Смотреть ТВ онлайн бесплатно | Прямые эфиры каналов`;
  updateMeta(
    `${_sn} — Смотреть ТВ онлайн бесплатно`,
...
function applyWatchSeoFallback(slug) {
  ...
  const _sn = window.SITE_NAME || window.location.hostname;
  const title = `${normalizedName} — смотреть онлайн прямой эфир | ${_sn}`;
  const description = `Смотрите ${normalizedName} онлайн бесплатно на ${_sn}. Прямой эфир без регистрации.`;
